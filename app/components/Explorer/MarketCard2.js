...
class MarketCard2 extends React.Component {
    static propTypes = {
    ...
    render() {
        ...
        let symbol1= quoteAsset.get("symbol");
        let symbol2= baseAsset.get("symbol");

        let symbol = symbol1+"_"+symbol2;
        let change = this.state.change;
        let high = this.state.high.toFixed(5);
        let low = this.state.low.toFixed(5);
        let last = utils.price_text(this.state.latestPrice, quoteAsset, baseAsset);
        let vol = utils.format_volume(this.state.volumeQuote);
        ...
        var url2 = "https://gt.finchain.info/match.php";
        fetch(url2,{
            method:"POST",
            headers: {},
            body:formData,
...
MarketCard2 = BindToChainState(MarketCard2, {keep_updating: true, show_loader: true});
